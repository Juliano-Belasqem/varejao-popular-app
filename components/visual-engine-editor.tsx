...
import {createVisualDocument,type VisualDocument,type VisualElement} from "@/lib/visual-engine";
...
export function VisualEngineEditor(){
 ...
 function patchTransform(key:keyof VisualElement["transform"],value:number){if(!current)return;patch({transform:{...current.transform,[key]:value}})}
 ...
 return <div style={{display:"grid",gridTemplateColumns:"220px minmax(0,1fr) 280px",gap:14,alignItems:"start"}}>
  ...
  <aside className="card"><strong>Propriedades</strong>{current?<div className="form" style={{marginTop:10}}><label className="field"><span>Nome</span><input className="input" value={current.name} onChange={e=>patch({name:e.target.value})}/></label><div style={{display:"flex",gap:10}}><label><input type="checkbox" checked={current.visible} onChange={e=>patch({visible:e.target.checked})}/> Visível</label><label><input type="checkbox" checked={current.locked} onChange={e=>patch({locked:e.target.checked})}/> Bloqueado</label></div>
   ...
   {current.type==="text"&&<><label className="field"><span>Texto livre</span><input className="input" value={current.text??""} onChange={e=>patch({text:e.target.value})}/></label><label className="field"><span>Vínculo de dados</span><input className="input" value={current.binding??""} onChange={e=>patch({binding:e.target.value||undefined})} placeholder="offer.price"/></label></>}
