...
export default async function OffersPage(){
 ...
 <section className="card">{error?<div className="error">A Central de Ofertas será liberada após a migração desta versão.</div>:!offers?.length?<div className="empty">Nenhuma oferta cadastrada.</div>:<div style={{overflowX:"auto"}}><table className="table"><thead><tr><th>Produto</th><th>Imagem</th><th>Normal</th><th>Oferta</th><th>Período</th><th>Campanha</th><th>Status</th><th></th></tr></thead><tbody>{offers.map((o:any)=><tr key={o.id}><td><strong>{o.products?.name??"Produto"}</strong><div className="muted">{[o.products?.brand,o.products?.specification].filter(Boolean).join(" · ")}</div></td><td><span className="pill">{withImage.has(o.product_id)?"Com imagem":"Sem imagem"}</span></td><td>{price(o.normal_price)}</td><td><strong>{price(o.offer_price)}</strong>{o.unit?` /${o.unit}`:""}</td><td>{date(o.starts_on)} — {date(o.ends_on)}</td><td>{o.campaigns?.name??"—"}</td><td><span className="pill">{o.active?"Ativa":"Inativa"}</span></td><td>{editable&&<form action={setOfferActive}><input type="hidden" name="id" value={o.id}/><input type="hidden" name="active" value={String(!o.active)}/><button className="btn">{o.active?"Desativar":"Ativar"}</button></form>}</td></tr>)}</tbody></table></div>}</section>
 <div style={{marginTop:12}} className="muted">Os materiais visuais serão ligados a esta oferta nas próximas etapas do motor. <Link href="/app/campanhas">Campanhas atuais continuam funcionando normalmente.</Link></div>
 </>;
}
