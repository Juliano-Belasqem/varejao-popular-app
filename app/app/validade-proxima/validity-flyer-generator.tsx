...
function ResponsivePrice({value,unit}:{value:string;unit:string}){const price=splitPrice(value);const scale=price.major.length<=1?1.22:price.major.length===2?1:price.major.length===3?.82:.68;return <div style={{display:"flex",alignItems:"flex-start",justifyContent:"center",height:"100%",lineHeight:.9,transform:`scale(${scale})`,transformOrigin:"center center"}}><small style={{fontSize:".2em",marginTop:".15em",color:"#c7192b"}}>R$</small><strong style={{fontSize:"1em"}}>{price.major}</strong><span style={{fontSize:".4em"}}>,{price.minor}<small style={{display:"block",fontSize:".55em",color:"#c7192b"}}>{unit}</small></span></div>}
...
function Barcode({code}:{code:string}){const bits=eanBars(code);const digits=validEan13(code);if(!bits||!digits)return <div className="barcode-fallback">{code||"EAN-13 inválido"}</div>;return <div style={{background:"#fff",padding:"2% 4%"}}><svg className="barcode-svg" style={{height:"1.15em",minHeight:32}} viewBox="0 0 113 60" preserveAspectRatio="xMidYMid meet" shapeRendering="crispEdges" aria-label={digits}><rect width="113" height="60" fill="#fff"/>{bits.split("").map((b,i)=>b==="1"?<rect key={i} x={11+i} y="2" width="1" height={(i<3||(i>=45&&i<50)||i>=92)?46:41} fill="#000"/>:null)}</svg><div className="barcode-number" style={{color:"#000",fontSize:".42em",lineHeight:1,letterSpacing:".12em",textAlign:"center"}}>{digits}</div></div>}
...
export function ValidityFlyerGenerator({products}:{products:Product[]}){
  ...
  return <>
    ...
    <div className="no-print" style={{marginBottom:18}}><section className="card" style={{position:"sticky",top:20}}><div className="section-title-row"><div><small className="eyebrow">PRÉVIA AO VIVO</small><h2>Template Mestre</h2></div></div><div style={{aspectRatio:"1 / 1.414",maxHeight:560,margin:"14px auto 0"}}><ConfiguredTicket config={template.config} fonts={fieldFonts} logoUrl={logoUrl} values={{title:"OFERTA",product:"PRODUTO EXEMPLO",brand:"MARCA",specification:"500G",validity:"VALIDADE 30/09/26",price:<div style={{display:"flex",alignItems:"flex-start",justifyContent:"center",height:"100%",lineHeight:.9}}><small style={{fontSize:".2em",marginTop:".15em",color:"#c7192b"}}>R$</small><strong style={{fontSize:"1em"}}>12</strong><span style={{fontSize:".4em"}}>,99<small style={{display:"block",fontSize:".55em",color:"#c7192b"}}>UN</small></span></div>,normalPrice:"15,99",code:<Barcode code="7891000100103"/>,footer:"Oferta válida enquanto durarem os estoques"}}/></div></section></div>
    ...
    <div className="validity-sheet-wrap"><div className="validity-sheet">{entries.map((e,index)=><ConfiguredTicket key={index} config={template.config} fonts={fieldFonts} logoUrl={logoUrl} values={{
      ...
      price:<ResponsivePrice value={e.offerPrice} unit={e.unit||"UN"}/>,
