...
type OpenFactsCandidate = { imageUrl: string | null; productName: string | null; brands: string | null; error: string | null };
type GoogleImageCandidate = { original: string; thumbnail: string; title: string; source: string; width: number | null; height: number | null };
type GoogleImageSearch = { results: GoogleImageCandidate[]; error: string | null };
type GoogleImageFilters = { transparent: boolean; type: string; ratio: string; size: string };
...
async function findOpenFactsImage(ean: string): Promise<OpenFactsCandidate> {
  ...
  try {
    ...
    if (!imageUrl) return { imageUrl: null, productName: candidate.product_name || null, brands: candidate.brands || null, error: "O produto foi encontrado no Open Food Facts, mas não possui imagem disponível." };
...
async function findGoogleImages(query: string, filters: GoogleImageFilters): Promise<GoogleImageSearch> {
  ...
  try {
    ...
    const payload = await response.json() as {
      error?: string;
      images_results?: Array<{ original?: string; thumbnail?: string; title?: string; source?: string; original_width?: number; original_height?: number }>;
    };
    ...
    const results = (payload.images_results ?? [])
      ...
      .slice(0, 12)
      .map((item) => ({
        original: item.original!, thumbnail: item.thumbnail!, title: item.title || "Imagem do produto", source: item.source || "Google Imagens",
        width: item.original_width ?? null, height: item.original_height ?? null,
      }));
...
export default async function ProductDetailPage({ params, searchParams }: {
  ...
  searchParams?: Promise<{
    image_source?: string; image_q?: string; image_trans?: string; image_type?: string; image_ratio?: string; image_size?: string;
    image_import_error?: string; image_import_ok?: string;
  }>;
}) {
  ...
  const googleFilters: GoogleImageFilters = {
    transparent: query.image_trans === "1", type: query.image_type || "", ratio: query.image_ratio || "", size: query.image_size || "",
  };
  ...
  return (
    <>
      ...
      {query.image_import_error ? <div className="error" style={{ marginBottom: 16 }}>{query.image_import_error}</div> : null}
      {query.image_import_ok ? <div className="card" style={{ marginBottom: 16, padding: 12 }}><strong>{query.image_import_ok}</strong></div> : null}
      ...
      <div className="grid" style={{ marginBottom: 16 }}>
        ...
        {editable && <section className="card">
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 12, alignItems: "start" }}>
            ...
            <GoogleImageSearchForm defaultQuery={googleQuery} initialTransparent={googleFilters.transparent} initialType={googleFilters.type} initialRatio={googleFilters.ratio} initialSize={googleFilters.size} />
          </div>
          {openFacts?.error && <div className="error">{openFacts.error}</div>}
          {openFacts?.imageUrl && <div className="card" style={{ padding: 12 }}>
            <div style={{ aspectRatio: "1 / 1", background: "#fff", borderRadius: 10, overflow: "hidden", display: "grid", placeItems: "center", maxWidth: 320 }}><img src={openFacts.imageUrl} alt={openFacts.productName || product.name} style={{ width: "100%", height: "100%", objectFit: "contain" }} /></div>
            <div style={{ marginTop: 10 }}><strong>{openFacts.productName || product.name}</strong>{openFacts.brands ? <div className="muted" style={{ marginTop: 4 }}>Marca no Open Food Facts: {openFacts.brands}</div> : null}</div>
            <form action={importOpenFactsImage} style={{ marginTop: 10 }}><input type="hidden" name="product_id" value={product.id} /><input type="hidden" name="source_url" value={openFacts.imageUrl} /><button className="btn primary" type="submit">Usar esta imagem</button></form>
          </div>}
        </section>}
      </div>

      {editable && googleImages && <section className="card" style={{ marginBottom: 16 }}>
        ...
        {googleImages.error ? <div className="error">{googleImages.error}</div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(190px,1fr))", gap: 14 }}>
          {googleImages.results.map((candidate, index) => <article className="card" key={`${candidate.original}-${index}`} style={{ padding: 10 }}>
            ...
            <form action={importSerpApiImage} style={{ marginTop: 8 }}>
              <input type="hidden" name="product_id" value={product.id} />
              <input type="hidden" name="source_url" value={candidate.original} />
              <input type="hidden" name="thumbnail_url" value={candidate.thumbnail} />
      ...
      <section className="card">
        ...
        {!signedImages.length ? <div className="empty">Nenhuma imagem aprovada para este produto ainda.</div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(210px,1fr))", gap: 16 }}>
          {signedImages.map((image) => <article className="card" key={image.id} style={{ padding: 12 }}>
            <div style={{ aspectRatio: "1 / 1", background: "#fff", borderRadius: 10, overflow: "hidden", display: "grid", placeItems: "center" }}>{image.signedUrl ? <img src={image.signedUrl} alt={product.name} style={{ width: "100%", height: "100%", objectFit: "contain" }} /> : <span className="muted">Imagem indisponível</span>}</div>
